...
public class SILab2 {
    public static boolean checkCart(List<Item> allItems, int payment){//1
        ...
        for (int i = 0; i < allItems.size(); i++){ //5.1 5.2 5.3
            ...
            if (item.getName() == null || item.getName().length() == 0){//7 //IF2
                item.setName("unknown"); //8
            }
            if (item.getBarcode() != null){ //9 //IF3
            ...
            if (item.getPrice() > 300 && item.getDiscount() > 0 && item.getBarcode().charAt(0) == '0'){ //21 //IF6
                sum -= 30; //22
            }
        } //23
        if (sum <= payment){ //24 //IF7
            return true; //25
        }
        else {
            return false; //26
